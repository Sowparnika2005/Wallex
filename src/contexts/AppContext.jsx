...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (isDemo || !isFirebaseConfigured()) {
      ...
      const savedTx = localStorage.getItem('fintracker-transactions');
      const savedCat = localStorage.getItem('fintracker-categories');
      const savedSettings = localStorage.getItem('fintracker-settings');
  ...
  useEffect(() => {
    if (isDemo || !isFirebaseConfigured()) {
      localStorage.setItem('fintracker-transactions', JSON.stringify(transactions));
    }
  }, [transactions, isDemo]);

  useEffect(() => {
    if (isDemo || !isFirebaseConfigured()) {
      localStorage.setItem('fintracker-categories', JSON.stringify(categories));
    }
  }, [categories, isDemo]);

  useEffect(() => {
    if (isDemo || !isFirebaseConfigured()) {
      localStorage.setItem('fintracker-settings', JSON.stringify(settings));
    }
  }, [settings, isDemo]);
